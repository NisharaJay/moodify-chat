...
export default function MoodifyChat() {
  const [messages, setMessages] = useState([{
    type: "bot",
    text: "Hey there! 🎵 I'm Moodify, your personal music mood companion. Tell me how you're feeling, and I'll recommend the perfect songs for your vibe!",
    timestamp: new Date(),
  }]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-950 to-black">
      <div className="container mx-auto px-4 py-8 max-w-4xl text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          {quickMoods.map((m) => (
            <button key={m.label} onClick={() => sendMessage(m.query)}
              className="bg-white/10 border border-white/20 rounded-full px-4 py-2 text-white hover:bg-white/20 transition-all">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[80%]">
                  <div className={`rounded-xl p-3 ${msg.type === "user" ? "bg-purple-500 text-white" : "bg-white/20 text-white border border-white/30"}`}>
                    ...
                    {msg.songs?.length > 0 && (
                      <div className="mt-3 space-y-2">
                        {msg.songs.map((s, j) => (
                          <a key={j} href={s.spotify_url} target="_blank" rel="noopener noreferrer"
                            className="flex items-center gap-3 p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-all">
                  ...
                  <p className="text-xs text-purple-300 mt-1 px-2">{msg.timestamp.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                </div>
              </div>
            ))}
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white/20 rounded-xl p-3 flex gap-1">
                  {[0, 1, 2].map((n) => (
                    <div key={n} className="w-2 h-2 bg-purple-300 rounded-full animate-bounce" style={{ animationDelay: `${n * 0.2}s` }}></div>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="border-t border-white/20 p-3 bg-white/5">
            <div className="flex gap-2">
              ...
              />
              <button onClick={() => sendMessage(input)} className="bg-purple-500 hover:bg-purple-600 text-white rounded-xl px-4 py-2 transition-all">
