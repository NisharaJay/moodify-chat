    ...
    <div className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
      <div className="max-w-[80%]">
        <div className={`rounded-xl p-3 ${msg.type === "user" ? "bg-purple-500 text-white" : "bg-white/20 text-white border border-white/30"}`}>
          ...
          {msg.songs?.length > 0 && msg.songs.map((s, i) => (
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl text-center">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden">
          <div className="h-[500px] overflow-y-auto p-4 space-y-3">
            ...
            {isTyping && <div className="flex justify-start"><div className="bg-white/20 rounded-xl p-3 flex gap-1">{[0,1,2].map(n => <div key={n} className="w-2 h-2 bg-purple-300 rounded-full animate-bounce" style={{ animationDelay: `${n*0.2}s` }}></div>)}</div></div>}
            <div ref={endRef} />
          </div>
